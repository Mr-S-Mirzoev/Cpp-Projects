...
std::pair<Time, int> dijkstra(int n, std::vector<Edge> const& edges, NodeID destination,
                              NodeID initial = 0)
{
    ...
    // set.
    std::unordered_map<NodeID, std::pair<Time, int>>
        unvisited; // [NodeID]->{min time to node from start; num min paths}
    unvisited.reserve(n);

    /* 2a. Assign to every node a tentative distance value: set it to zero for our initial node and
    to infinity for all other nodes. The tentative distance of a node v is the length of the
    shortest path discovered so far between the node v and the starting node. Since initially no
    path is known to any other vertex than the source itself (which is a path of length zero), all
    ...
    while (1)
    {
        ...
        with a distance greater than 8 then change it to 8. Otherwise, the current value will be
        kept. */
        ...
        /* 4. When we are done considering all of the unvisited neighbors of the current node, mark
...
in how many ways you can travel from intersection 0 to intersection n - 1 in the shortest amount of
time.
