...
class PrefixTree
{
    struct PrefixTreeNode
    {
        ...
        virtual void inc_count() { throw std::logic_error("Unimplemented for non-leaf nodes"); }
    };

    struct TerminatingPrefixTreeNode : PrefixTreeNode
    {
        unsigned get_count() const final { return count; }
        void inc_count() final { ++count; }
    ...
    void add_sub(std::string const& s)
    {
        ...
        for (unsigned i = 0; i < s.size(); ++i)
        {
            ...
            if (it == cur->children.end())
            {
                ...
                auto insertion_result = cur->children.insert({s[i], newnode});
...
public:
    PrefixTree(std::vector<std::string> const& subs) : tree_(new PrefixTreeNode), N_(subs.size())
    {
        if (!N_)
    ...
    unsigned full_substring_len() const { return subs_len_ * N_; }
