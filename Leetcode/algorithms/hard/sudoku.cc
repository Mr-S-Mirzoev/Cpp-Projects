...
constexpr short to_digit(char c) { return c - '0'; }
constexpr char to_char(short v) { return v == 0 ? '.' : v + '0'; }
...
public:
    ...
    */
    BitSet() { storage.resize(STORAGE_SIZE); }
    BitSet(BitSet const& other) { storage = other.storage; }
    BitSet(BitSet&& other) { storage = std::move(other.storage); }
        ...
        for (unsigned i = 0; i < N_; ++i)
        {
            ...
            for (unsigned j = 0; j < M_; ++j)
            {
                ...
                if (char_board[i][j] == '.')
                    ...
                    throw std::logic_error("Unknown char in board representation: " +
                                           std::to_string(char_board[i][j]));
...
private:
    ...
    {
        auto merge =
            grid_vals_[(i / 3) * (M_ / 3) + (j / 3)].merge(row_vals_[i].merge(col_vals_[j]));
