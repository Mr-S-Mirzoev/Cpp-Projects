...
// KB to B
unsigned long long operator"" _KB(unsigned long long kb) { return kb << 10; }
...
int main(int argc, char* argv[])
{
    ...
    if (ret_val)
    {
        std::cout << "Failed to rename run to " << output_name << ": " << strerror(ret_val)
                  << std::endl;
        return 1;
    }
