...
public:
    LineFromRun(std::string line, int run_id) : line_(line), run_id_(run_id) {}

    bool operator<(const LineFromRun& rhs) const { return (line_.compare(rhs.line_) > 0); }

    std::string get_line() const { return line_; }

    int get_run_id() const { return run_id_; }
};
