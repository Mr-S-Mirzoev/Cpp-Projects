...
public:
    ...
    // if isWait == true, all the functions in the queue are run, otherwise the queue is cleared
    ...
    void stop(bool isWait = false)
    {
        ...
        // deleted by the threads therefore delete them here
        this->clear_queue();
        this->threads.clear();
        this->flags.clear();
    }
    ...
    // run the user's function that excepts argument int - id of the running thread. returned value
    // is templatized operator returns std::future, where the user can get the result and rethrow
        ...
        auto f = [this, i, flag /* a copy of the shared ptr to the flag */]()
        {
            ...
            while (true)
            {
                ...
                { // if there is anything in the queue
                    ...
                    if (_flag)
                        return; // the thread is wanted to stop, return even if the queue is not
                                // empty yet
                    else
                        isPop = this->q.pop(_f);
                }
