...
class Matrix
{
    ...
    */
    ...
    static void multiplication_rows(Matrix& c, Matrix& a, Matrix& b, unsigned i_start,
                                    unsigned i_finish)
    {
...
public:
    Matrix(unsigned h, unsigned l, bool random = false, T min = T(), T max = T())
        : l(l), h(h), rows(new std::vector<T>[h])
    {
    ...
    static Matrix<T> multiply(Matrix<T>& a, Matrix<T>& b, unsigned threads_q = 1)
    {
        ...
        if (threads_q)
        {
            ...
            unsigned quantity =
                (c.h % load) ? unsigned(floor(c.h / load)) + 1 : unsigned(floor(c.h / load));
            std::vector<std::future<void>> results(quantity);
            unsigned k = 0;
            for (unsigned i = 0; i < c.h; i += load)
            {
                results[k] = p.push(std::bind(multiplication_rows, std::ref(c), std::ref(a),
                                              std::ref(b), i, (i + 10 < c.h) ? i + 10 : c.h));
                ++k;
            }
