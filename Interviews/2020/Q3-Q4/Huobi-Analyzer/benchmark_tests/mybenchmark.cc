...
generateRandomUpdate(double min, double max, bool noNulls, std::vector<double>& cvalues)
{
    ...
    std::sort(v.begin(), v.end(),
              [](const std::pair<double, unsigned long>& a,
                 const std::pair<double, unsigned long>& b) { return a.first < b.first; });
    return v;
}
    ...
    for (auto _ : state)
    {
        ...
        */
        ...
        InsertVectorsonLists(a, b, asks, bids, flag)
            ...
            state.PauseTiming();
        FindMaxonLists(ba_price, ba_amount, bb_price, bb_amount, asks, bids) state.ResumeTiming();
    }
}
...
static void BM_Insert_On_Maps(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        InsertVectorsonMaps(a, b, asks, bids, flag)
            ...
            state.PauseTiming();
        FindMaxonMaps(ba_price, ba_amount, bb_price, bb_amount, asks, bids) state.ResumeTiming();
    }
}
// Register the function as a benchmark
...
static void BM_Insert_On_UnorderedMaps(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        vals.clear();
        state.ResumeTiming();
        InsertVectorsonUnorderedMaps(a, b, asks, bids, flag, bid_min_invalidated,
                                     ask_max_invalidated, bids_min, bids_amount, asks_max,
                                     asks_amount) state.PauseTiming();
        FindMaxonUnorderedMaps(bids_min, bids_amount, asks_max, asks_amount, asks, bids,
                               bid_min_invalidated, ask_max_invalidated)
...
static void BM_FindMax_On_Lists(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        InsertVectorsonLists(a, b, asks, bids, flag)
            ...
            state.ResumeTiming();
        FindMaxonLists(ba_price, ba_amount, bb_price, bb_amount, asks, bids) state.PauseTiming();
    }
    //	std::cout << max << std::endl;
}
// Register the function as a benchmark
...
static void BM_FindMax_On_Maps(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        InsertVectorsonMaps(a, b, asks, bids, flag) state.ResumeTiming();
...
static void BM_FindMax_On_UnorderedMaps(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        InsertVectorsonUnorderedMaps(a, b, asks, bids, flag, bid_min_invalidated,
                                     ask_max_invalidated, bids_min, bids_amount, asks_max,
                                     asks_amount) state.ResumeTiming();
        FindMaxonUnorderedMaps(bids_min, bids_amount, asks_max, asks_amount, asks, bids,
                               bid_min_invalidated, ask_max_invalidated) state.PauseTiming();
    }
    // std::cout << max << std::endl;
}
// Register the function as a benchmark
