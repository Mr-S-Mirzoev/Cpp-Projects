...
public:
    ...
    node* root = NULL;
    int n;
    void insert(T x, V v) { root = insertUtil(root, x, v); }
    void remove(T x) { root = removeUtil(root, x); }
    node* search(T x) { return searchUtil(root, x); }
    void inorder()
    {
        inorderUtil(root);
        cout << endl;
    ...
    node* min() { return minValue(root); }
    node* max() { return maxValue(root); }
...
private:
    ...
    node* insertUtil(node* head, T x, V v)
    {
        ...
        if (x < head->key)
            head->left = insertUtil(head->left, x, v);
        else if (x > head->key)
            head->right = insertUtil(head->right, x, v);
        head->height = 1 + std::max(height(head->left), height(head->right));
