...
int scalar_mul(std::vector <std::pair<int, unsigned>> const& a, std::vector <std::pair<int, unsigned>> const& b) {
    if (a.size() * b.size() == 0)
        return 0;
...
{
    // [{2, 3}, {1, 2}, {-1, 3}] * [{0, 4}, {5, 4}]
    std::vector <std::pair<int, unsigned>> a {
        {2, 3},
        {1, 2},
        {-1, 3}
    };
    std::vector <std::pair<int, unsigned>> b {
        {0, 4},
        {5, 4}
    };
