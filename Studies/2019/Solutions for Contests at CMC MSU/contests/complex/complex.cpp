...
public:
    ...
    const double abs2() const { return (Re * Re + Im * Im); }
    const double abs() const { return sqrt(this->abs2()); }
...
public:
    ...
    const complex operator+() const { return values[sz - 1]; }
    ...
    const complex operator[](const size_t i) const { return values[i]; }
    const complex_stack operator~() const { return this->shorten(sz - 1); }
