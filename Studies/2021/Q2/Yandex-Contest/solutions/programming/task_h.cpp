...
struct TimeStamp
{
    ...
    TimeStamp(unsigned _h = 0, unsigned _m = 0, unsigned _s = 0) : h(_h), m(_m), s(_s) {}
    ...
    constexpr bool operator<=(const TimeStamp& rhs) const { return (*this == rhs || *this < rhs); }
...
struct TimeBeam
{
    ...
    TimeBeam(const TimeStamp& ts, char sign) : point(ts) { carry = sign == '+' ? +1 : -1; }
...
int main(int argc, char const* argv[])
{
    ...
    for (auto& timebeam : timebeams)
    {
        ...
        // std::cout << timebeam.to_string() << " " << couriers << " " << max_couriers_at_a_time <<
        // std::endl;
    }
...
Let's use + to denote the start of a segment and − to denote the end. For each segment, create two
pairs, one for each endpoint:
...
You can do this in time O(NlogN) with any reasonable sorting algorithm, or in time O(N+K) using
key-indexed counting. In the example, we get:
...
Now process the endpoints in order. Maintain a count of the number of active segments, which is
initially 0. Every time you process a +, increase the count by 1. Every time you process a −,
decrease the count by 1. After processing each endpoint, check if the new count is higher than the
largest count so far; if it is, update your solution.
...
This second phase of the algorithm takes time proportional N. The whole algorithm takes time
O(NlogN) with a generic sort, or O(N+K) with key-indexed counting.
