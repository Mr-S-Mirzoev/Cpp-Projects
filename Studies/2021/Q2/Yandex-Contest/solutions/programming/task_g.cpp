...
struct Period
{
    ...
    Period(const TimeStamp& s, const TimeStamp& e) : start(s), end(e) {}

    constexpr bool operator<(const Period& rhs) const { return start < rhs.start; }

    std::string to_string() const { return start.to_string() + " " + end.to_string(); }
};
...
int main(int argc, char const* argv[])
{
    ...
    while (std::getline(input, line))
    {
        ...
        else
        {
            ...
            orders.push_back(Period(TimeStamp(start_s), TimeStamp(end_s)));
        }
    }
