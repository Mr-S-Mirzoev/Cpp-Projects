...
{
public:
    SizeMismatchException(const std::string& op, std::size_t n1, std::size_t m1, std::size_t n2,
                          std::size_t m2)
        : LinearAlgebraException("Sizes mismatch in" + op + ": " + std::to_string(n1) + "x" +
                                 std::to_string(m1) + " " + op + " " + std::to_string(n2) + "x" +
                                 std::to_string(m2))
    {
    }
...
{
public:
    NotAPairException(const std::string& s) : LinearAlgebraException("Not a pair! " + s) {}
...
{
public:
    ZeroDeterminantException() : LinearAlgebraException("The determinant of matrix is zero") {}
}; // class ZeroDeterminantException

class IndexOutOfRangeException : public LinearAlgebraException
{
public:
    IndexOutOfRangeException(std::size_t i, std::size_t j, std::size_t n, std::size_t m)
        : LinearAlgebraException("Index out of range. Trying to access [" + std::to_string(i) +
                                 "," + std::to_string(j) + "] in a " + std::to_string(n) + "x" +
    ...
    IndexOutOfRangeException(std::size_t i, std::size_t n)
        : LinearAlgebraException("Index out of range. Trying to access [" + std::to_string(i) +
                                 "] in a " + std::to_string(n) + "-sized vector.")
    {
    }
...
{
public:
    IrrationalSpaceUsageException() : LinearAlgebraException("The space is being used irrationally")
    {
    }
