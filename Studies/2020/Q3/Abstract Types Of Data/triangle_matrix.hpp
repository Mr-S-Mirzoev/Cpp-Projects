...
public:
            ...
            if (upper)
            {
                if (i > j)
                {
                    throw std::logic_error(
                        "Trying to set element of upper triangle matrix where i > j");
                }
            }
            else
            {
                ...
                {
                    throw std::logic_error(
                        "Trying to set element of lower triangle matrix where i < j");
                }
            }
        }
        if (i < 0 || i >= this->size || j < 0 || j >= this->size)
    ...
    bool GetIsUpper() const { return this->upper; }
