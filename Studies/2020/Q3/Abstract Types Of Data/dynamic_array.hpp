...
public:
    ...
    DynamicArray(T* items, int size) : size(size)
    {
        if (size < 0)
        {
            throw std::logic_error("Trying to make an array with a negative size: " +
                                   std::to_string(size));
        }
        else if (size == 0)
        {
    ...
    {
        if (index >= size)
            throw Lab::IndexOutOfRange("Went out of range, requesting " + std::to_string(index) +
                                       "-th element in array with length " + std::to_string(size));
        if (index < 0)
            throw Lab::IndexOutOfRange(
                "Went out of range, requesting element with a negative index");
        return arr[index];
    }
    ...
    void Set(int index, T value)
    {
        if (index < 0)
            throw Lab::IndexOutOfRange(
                "Went out of range, trying to set value to a negative index");
        if (index >= size)
    ...
    void Resize(int newSize)
    {
        if (newSize == size)
            return;
        if (newSize < 0)
            throw std::logic_error("Trying to resize the array to a negative size: " +
