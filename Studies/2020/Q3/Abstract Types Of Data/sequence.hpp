...
public:
    ...
    virtual T GetFirst() const override { return this->arr->Get(0); }
    virtual T GetLast() const override { return this->arr->Get(this->size - 1); }
    virtual T Get(int index) const override { return this->arr->Get(index); }
    ...
    virtual void InsertAt(const T& item, int index) override
    {
        ...
        if (index >= this->size)
            throw Lab::IndexOutOfRange("Went out of range, trying to set " + std::to_string(index) +
                                       "-th element in array with length " +
                                       std::to_string(this->arr->GetSize()));
        this->arr->Resize(this->size + 1);
    ...
    virtual void Print() const override { std::cout << *(this->arr) << std::endl; }
    DynamicArray<T>* getptr() { return arr; }
};
...
public:
    ...
    virtual T GetFirst() const override { return this->list->GetFirst(); }
    virtual T GetLast() const override { return this->list->GetLast(); }
    virtual T Get(int index) const override { return this->list->Get(index); }
    ...
    virtual void Set(const T& item, int index) override { this->list->Set(item, index); }
    virtual void Append(const T& item) override
    {
    ...
    virtual void Print() const override { std::cout << *(this->list) << std::endl; }
};
} // namespace Lab